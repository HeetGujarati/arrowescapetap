...
class _MainMenuScreenState extends State<MainMenuScreen> {
  ...
  Future<void> _checkNotificationPermission() async {
    if (!mounted) return;
    final progress = context.read<ProgressRepository>();
    if (progress.highestUnlockedLevel >= 2 && progress.notificationsEnabled) {
      try {
        final prefs = await SharedPreferences.getInstance();
        final hasAsked = prefs.getBool('has_prompted_notification') ?? false;
        if (!hasAsked) {
          await prefs.setBool('has_prompted_notification', true);
          await NotificationService.instance.requestPermission();
        }
      } catch (e) {
        debugPrint('Error prompting notification permission: $e');
      }
    }
  }
        ...
        child: Stack(
          children: [
            ...
            SafeArea(
              ...
              // ── Daily Streak Badge (Centered under Title) ──
